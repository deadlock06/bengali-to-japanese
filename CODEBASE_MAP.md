...
- **Romaji weaning + advanced backbone** (D-037): Settings→শেখার ধরন "রোমাজি দেখাও" switch (classroom readings + vocab bank honor it; writing screen keeps romaji); `tools/import_jlpt_list.py` imported the open JLPT deck → **n3:3665 · n2:3414 · n1:4931** whitelist forms — validator bounds every level.
- **Re-audit clean** (D-038): 6 passes green; orphan agent_panel deleted; .tts-cache untracked; **version → 0.9.0+2** (was stale 0.1.0 "Internal SENSEI").
- **Design-system pass** (D-039): icon 2.0 (gradient tile + red-sun motif + shadowed ば + ringed dot, rounded web variants); rounded icon family + NavigationBar selectedIcon states; theme-level unified TextField (yellow focus)/ListTile/ExpansionTile/ProgressIndicator/Tooltip. build_apk.sh lock-leak into adb FIXED (fd 9 closed on all flutter calls).
