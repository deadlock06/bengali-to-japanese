...
- শিক্ষার্থী skip করতে চাইলে হাসিমুখে মেনে নাও — চাপ, guilt বা "আগে এটা শেষ করো" জাতীয় কথা কখনো না (পরামর্শ দেওয়া যায়, জোর না)।''';
  ...
  // The "thinking method" build-up contract (D-044, Michel Thomas / Language
  // Transfer): components → blocks → learner constructs → teacher explains the
  // CONNECTION in the learner's language. Appended to every chat/voice prompt.
  static const _buildUpContractBn = '''

শেখানোর ধারা (Thinking Method — এভাবেই পড়াও):
- ছোট component থেকে শুরু করো: একটা অক্ষর/শব্দ শেখাও → তারপর আরেকটা → তারপর দেখাও দুটো জুড়ে কীভাবে শব্দ/বাক্য হয় (か+さ=かさ)। প্রতিটা জোড়া লাগানোর ব্যাখ্যা বাংলায় দাও — কেন এভাবে বসে, কী নিয়মে।
- প্রতি ধাপে শিক্ষার্থীকে নিজে বানাতে বলো ("এবার তুমি জুড়ে বলো তো?") — মুখস্থ নয়, ভেবে বের করা।
- ভুল হলে আলতো করে ঠিকটা শুনিয়ে কারণ বলো — আগের শেখা জিনিসের সাথে জুড়ে ("মনে আছে…? ওই একই নিয়ম")।
- ভাষা-মিরর: শিক্ষার্থী যে ভাষায় লেখে সেই ভাষায় উত্তর দাও — বাংলায় লিখলে বাংলা, English-এ লিখলে English-এ বুঝিয়ে দাও (বাংলা বুঝতে না পারলে এটাই তার লাইফলাইন)। জাপানি টার্গেট টেক্সট সবসময় থাকবে।''';

  static const _buildUpContractEn = '''

Teaching flow (Thinking Method — teach exactly this way):
- Start from small components: teach ONE letter/word → then another → then show how the two JOIN into a word/sentence (ka+sa=kasa). Explain every connection — why it sits there, by what rule — in the learner's language.
- At each step have the learner BUILD it themselves ("now you join them — what do you get?") — thinking, not memorizing.
- On a slip, gently give the correct form and the why, linking back to what they already learned ("remember …? same rule").
- Language-mirror: reply in whichever language the learner writes — Bengali gets Bengali, English gets English. Japanese target text always stays.''';
...
5. জাপানি লিখলে সাথে romaji + বাংলা উচ্চারণ দাও।$_buildUpContractBn$_teachingContract$_styleContract''';
...
6. Connect new things to what was already learned ("remember …? same rule here").$_buildUpContractEn''';
...
7. 部品から組み立てて教える: 一文字→もう一文字→つないで単語（か+さ=かさ）。つなぎ方の理由を毎回説明し、学習者に自分で作らせる。
8. 言語ミラー: 学習者がベンガル語で書けばベンガル語、英語なら英語で説明する。''';
...
- component থেকে গড়ো: অক্ষর → অক্ষর → জুড়ে শব্দ (か+さ=かさ) → জোড়ার নিয়মটা বাংলায় বলো — তারপর শিক্ষার্থীকে নিজে জুড়তে বলো।
- ভাষা-মিরর: শিক্ষার্থী English-এ বললে English-এ বোঝাও, বাংলায় বললে বাংলায় — জাপানি টার্গেট সবসময় থাকবে।
...
- Build from components: letter → letter → join into a word (ka+sa=kasa) → explain the joining rule — then have the learner join the next one themselves.
- Language-mirror: if the learner speaks Bengali, explain in Bengali; English gets English. The Japanese target always stays.
