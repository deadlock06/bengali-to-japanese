...
## ⚡ 2026-07-18 (Opus 4.8) — APK SHIPPED · N1 ladder scaffolded · voice+brand+AI overhaul (all committed+pushed)
- **START HERE:** `node tools/progress_scale.mjs` — all proofs green. 23-unit ladder L0→N1 now in curriculum.json.
- **D2 DONE ON OWNER MACHINE:** `tools/build_apk.sh` (one-shot: local JDK21 + Android SDK + flock/resume-safe downloads) → **app-release.apk 87MB built + installed on owner's TECNO** (~/Downloads/bhasago.apk; also served at /bhasago.apk by web_server).
- **This session shipped:** D-026 no-fake-data UX pass (coin-flip AI check → real MockExam; real progress chips/skills; dead code removed) · D-027 N4 mock = official blueprint times · **D-028 N5→N1 scaffolding** (levels+mock units, `item_type` schema both sides, level whitelists, level-parameterized mock builder w/ official pass marks, kindForUnit; N-levels HONESTLY "content coming" until authored) · **D-029 Banglish style contract** in every LLM prompt (translated-sounding AI fixed at the register level) · **D-030 Teaching Philosophy** (docs/14, binding; taught-scope hint from live curriculum now sent with EVERY chat call) · **D-031 tiered AI routing** (quick→cheap chain; teach→Claude opus-4-8 native → Gemini Pro → GPT-4o; keys env-only; mastery 🏆 আয়ত্তে on Progress) · **D-032 brand+audio** (real ば launcher/web icons replace default Flutter logo, app label/title/manifest → Bhasago, dark boot splash web+Android, ALL audio clips regenerated at learner rate -12%/+5Hz).
- **NEXT (ranked):** ① owner adds AI keys (ANTHROPIC_API_KEY/GEMINI_API_KEY/DEEPSEEK_API_KEY env on the proxy) — teach tier is dark until then ② rebuild APK (owner machine, `bash tools/build_apk.sh` — fast now) to pick up 6 decisions of updates ③ A5 native review (human-gated) ④ N3 whitelist + first N3 content drafts (verified:false until review) ⑤ D4/D5 device AI work ⑥ E1 privacy policy.
- **⚠️ pre-Play-Store:** applicationId is still `com.example.sensei_app` (store will reject com.example) — change in android/app/build.gradle.kts BEFORE first store upload, never after.
- **OWNER ACTIONS OPEN:** rotate Supabase DB password · revoke exposed OpenAI key · delete the GitHub push token in chat history · provide AI provider keys.
...
- **Also this session:** unified sensei chat everywhere (copy-anywhere → same chat, page-specific PERSISTENT history via `ChatHistoryStore`, chatKeys `lesson:<id>` / `kana:<char>` / `explain:<text>`); kana 46→71 (dakuten/handakuten + 46 new audio clips, 238 total); **writing practice is a lesson step** (Phase 3 kana: `writingPhase` + `KanaTracePad`, skip stays free); audio fix (fresh player per clip — web kept replaying the first clip); TTS Bengali-voice detection + honest no-voice message; orphan lessons wired (greetings/shopping/emergency → A1.1/A1.3/A2.2); explain prompt never refuses.
- **CONTENT SLICE (late session): stubs eliminated** — 68 new verified items (vocab 100→168, ladder 24%→33%), 3 new lessons (numbers_big/week/food) + 7 expanded to 8 items each; 306 audio clips; batch_reference 11/11 (21 lessons batchable). RULE: ≤8 items/lesson file (batch cap!) — grow units by ADDING lesson files to the unit's comma list. Cross-lesson repeat pairs (greetings↔work_intro etc.) are intentional reinforcement.
- **NEXT (vision gaps, ranked):** ① more A1/A2 lesson files toward the 772 ladder target (~+250 items; same authoring pattern as tools used 2026-07-16) ② real-conversation stage (scenario/roleplay layer) ③ voice-first STT (Tier-2 whisper alignment — device work) ④ vocab Phase-3 (say-it) + Phase-4 (word-blocks) ⑤ N4 content authoring (needs verified grammar + n4 whitelist) ⑥ yōon/sokuon/long-vowel kana ⑦ native-speaker review of ALL content (human-gated, before publish).
- **Verify:** `cd /home/marjuk/outputs/SENSEI && export PATH="$HOME/flutter/bin:$PATH"; flutter analyze` (clean) · `flutter test` (52/52) · web at localhost:5601 (launch config `sensei-web-release`). NOTE: shell cwd resets between commands — always cd first. A PARALLEL session added offline-chat fallback (content_repository.explainOffline/handleOfflineChat) — reconciled + committed; re-read files before editing, more parallel edits possible.
